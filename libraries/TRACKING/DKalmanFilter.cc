...
jerror_t DKalmanFilter::SetCDCForwardReferenceTrajectory(DMatrix &S){
  ...
  for (unsigned int m=0;m<cdchits.size();m++){
	...
	if (forward_traj_cdc.size()<3){ 
	  _DBG_ 
	    << "Too few steps before first CDC measurement!" << endl;
	  continue;
	}
...
jerror_t DKalmanFilter::KalmanLoop(double mass_hyp){
  ...
  if (cdchits.size()>0){  
    ...
    C0(state_q_over_pt,state_q_over_pt)=0.1*0.1*q_over_pt_*q_over_pt_;
...
jerror_t DKalmanFilter::KalmanForwardCDC(double mass_hyp,double anneal,
  ...
  for (unsigned int k=1;k<forward_traj_cdc.size()-1;k++){
	...
	+(z-STEP_SIZE-cdchits[id]->origin.z())*cdchits[id]->dir;
      ...
      if (DEBUG_LEVEL>0) 
	_DBG_ 
	  << "cosa " << cosa << " meas " << dm << " pred " << d 
	  << " p " << fabs(1./S(state_q_over_p,0))
	  << endl;
