//******************************************************************************
// DFactory_DFDCCathodeCluster.cc
//
...
///
DFactory_DFDCCathodeCluster::DFactory_DFDCCathodeCluster() {
	_logFile = new ofstream("DFactory_DFDCCathodeCluster.log");
	_log = new DStreamLog(*_logFile, "CLUST");
}
...
///
derror_t DFactory_DFDCCathodeCluster::evnt(DEventLoop *eventLoop, int eventNo) {
	...
	try {
		...
		cerr << "exception caught in DFactory_DFDCCathodeCluster" << endl;
	}
...
///
void DFactory_DFDCCathodeCluster::pique(vector<const DFDCHit*>& H) {
	if (H.size() == 0)
		return;
...
/// factory.
///
const string DFactory_DFDCCathodeCluster::toString() {
	if (_data.size() <= 0)
		return "";
