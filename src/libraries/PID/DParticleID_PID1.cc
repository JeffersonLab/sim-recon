...
jerror_t DParticleID_PID1::GetdEdxChiSq(const DTrackTimeBased *track, double &locActualdEdx, unsigned int &num, double &chi2, double &path){
  //initialization
  locActualdEdx=chi2=0.;
  num=0;
  path = 0.0;
  ...
  double dEdx=0.0, dEdx_diff=0.;
  double p_avg=0.;
  double mean_path_length=0.;
  num=dEdx_list.size()/2+1;
  double locTotaldE = 0.0, locTotaldx = 0.0;
  double N=double(num);
  ...
  dEdx/=N;
  dEdx_diff/=N;
  mean_path_length = path/N;
  p_avg/=N;
  locActualdEdx = locTotaldE/locTotaldx; 
