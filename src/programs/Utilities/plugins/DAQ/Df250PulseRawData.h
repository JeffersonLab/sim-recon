...
class Df250PulseRawData:public jana::JObject{
	...
	public:
		...
		Df250PulseRawData(uint32_t rocid=0, uint32_t slot=0, uint32_t channel=0, uint32_t itrigger=0, uint32_t pulse_number=0, uint32_t first_sample_number=0):rocid(rocid),slot(slot),channel(channel),itrigger(itrigger),pulse_number(pulse_number),first_sample_number(first_sample_number),invalid_samples(false),overflow(false){}
