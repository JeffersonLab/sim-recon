...
void TAGHSmearer::SmearEvent(hddm_s::HDDM *record)
{
   ...
   for (iter = taghs.begin(); iter != taghs.end(); ++iter) {
      ...
      for (titer = thits.begin(); titer != thits.end(); ++titer) {
         // smear the time
         double t = titer->getT() + gDRandom.SampleGaussian(tagh_config->TAGH_TSIGMA);
         double tADC = titer->getT() + gDRandom.SampleGaussian(tagh_config->TAGH_FADC_TSIGMA);
         double npe = gDRandom.SamplePoisson(titer->getDE() * tagh_config->TAGH_NPE_PER_GEV);
