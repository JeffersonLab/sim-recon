...
extern "C"
{
	void InitPlugin(JApplication *locApplication)
	{
		...
		locApplication->AddProcessor(new DEventProcessor_ee_convert()); //register this plugin
		locApplication->AddFactoryGenerator(new DFactoryGenerator_ee_convert()); //register the factory generator
	}
} // "C"
...
// init
//------------------
jerror_t DEventProcessor_ee_convert::init(void)
{
	// This is called once at program startup.

	/*
	//OPTIONAL: Create an EventStore skim.  
	string locSkimFileName = "ee_convert.idxa";
...
// brun
//------------------
jerror_t DEventProcessor_ee_convert::brun(jana::JEventLoop* locEventLoop, int32_t locRunNumber)
{
...
// evnt
//------------------
jerror_t DEventProcessor_ee_convert::evnt(jana::JEventLoop* locEventLoop, uint64_t locEventNumber)
{
	...
	locEventWriterROOT->Fill_DataTrees(locEventLoop, "ee_convert");
	...
	*/
	...
	for(size_t loc_i = 0; loc_i < locParticleCombos.size(); ++loc_i)
	{
		...
		if(locParticleCombo->Get_Reaction()->Get_ReactionName() != "ee_convert")
	...
	*/
	...
	for(size_t loc_i = 0; loc_i < locAnalysisResultsVector.size(); ++loc_i)
	{
		...
		if(locAnalysisResults->Get_Reaction()->Get_ReactionName() != "ee_convert")
	...
	*/
	...
	for(size_t loc_i = 0; loc_i < locAnalysisResultsVector.size(); ++loc_i)
	{
		...
		if(!locPassedParticleCombos.empty())
			locEventWriterREST->Write_RESTEvent(locEventLoop, "ee_convert"); //string is part of output file name
	}
	*/
	...
	for(size_t loc_i = 0; loc_i < locAnalysisResultsVector.size(); ++loc_i)
	{
		...
		japp->WriteLock("ee_convert.idxa"); //Lock is unique to this output file
		{
			dEventStoreSkimStream << locRunNumber << " " << locEventNumber << " " << locUniqueID << endl;
		}
		japp->Unlock("ee_convert.idxa");
	}
	*/
...
int DEventProcessor_ee_convert::Get_FileNumber(JEventLoop* locEventLoop) const
{
...
// erun
//------------------
jerror_t DEventProcessor_ee_convert::erun(void)
{
...
// fini
//------------------
jerror_t DEventProcessor_ee_convert::fini(void)
{
