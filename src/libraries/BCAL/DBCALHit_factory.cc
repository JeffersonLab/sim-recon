...
jerror_t DBCALHit_factory::evnt(JEventLoop *loop, uint64_t eventnumber)
{
   ...
   for(unsigned int i=0; i<digihits.size(); i++){
      ...
      // Copy values from digihit into local doubles
      double integral          = (double)digihit->pulse_integral;
      double nsamples_integral = (double)digihit->nsamples_integral;
      double nsamples_pedestal = (double)digihit->nsamples_pedestal;
      double single_sample_ped = (double)digihit->pedestal/nsamples_pedestal;

      double totalpedestal     = single_sample_ped * nsamples_integral;
