...
// init
//------------------
jerror_t DFDCHit_factory_Raw::init(void)
{
...
// brun
//------------------
jerror_t DFDCHit_factory_Raw::brun(jana::JEventLoop *eventLoop, int32_t runnumber)
{
   ...
   // now load them all
   if(print_messages) jout << "In DFDCHit_factory_Raw, loading constants..." << endl;
...
// evnt
//------------------
jerror_t DFDCHit_factory_Raw::evnt(JEventLoop *loop, uint64_t eventnumber)
{
...
// erun
//------------------
jerror_t DFDCHit_factory_Raw::erun(void)
{
   return NOERROR;
}
...
// fini
//------------------
jerror_t DFDCHit_factory_Raw::fini(void)
{
   return NOERROR;
}
...
//------------------
void DFDCHit_factory_Raw::LoadPackageCalibTables(jana::JEventLoop *eventLoop, string ccdb_prefix)
{
...
//------------------------------------
const double DFDCHit_factory_Raw::GetConstant(const fdc_digi_constants_t &the_table,
      const int in_gPlane, const int in_element) const {

   char str[256];

   if( (in_gPlane <= 0) || (static_cast<unsigned int>(in_gPlane) > FDC_NUM_PLANES)) {
      sprintf(str, "Bad gPlane # requested in DFDCHit_factory_Raw::GetConstant()! requested=%d , should be %ud", in_gPlane, FDC_NUM_PLANES);
      cerr << str << endl;
      throw JException(str);
   }
   ...
   if( (in_element <= 0) || (static_cast<unsigned int>(in_element) > the_table[in_gPlane].size())) {
      sprintf(str, "Bad element # requested in DFDCHit_factory_Raw::GetConstant()! requested=%d , should be %zu", in_element, the_table[in_gPlane].size());
      cerr << str << endl;
      throw JException(str);
   }
...
const double DFDCHit_factory_Raw::GetConstant(const fdc_digi_constants_t &the_table,
   ...
   if( (gPlane <= 0) || (static_cast<unsigned int>(gPlane) > FDC_NUM_PLANES)) {
      sprintf(str, "Bad gPlane # requested in DFDCHit_factory_Raw::GetConstant()! requested=%d , should be %ud", gPlane, FDC_NUM_PLANES);
      cerr << str << endl;
      throw JException(str);
   }
   // strip and wire planes have different numbers of elements
   if( (in_digihit->strip <= 0) || (static_cast<unsigned int>(in_digihit->strip) > STRIPS_PER_PLANE)) {
      sprintf(str, "Bad strip # requested in DFDCHit_factory_Raw::GetConstant()! requested=%d , should be %ud", in_digihit->strip, STRIPS_PER_PLANE);
      cerr << str << endl;
      throw JException(str);
   }
...
const double DFDCHit_factory_Raw::GetConstant(const fdc_digi_constants_t &the_table,
   ...
   if( (in_digihit->wire <= 0) || (static_cast<unsigned int>(in_digihit->wire) > WIRES_PER_PLANE)) {
      sprintf(str, "Bad wire # requested in DFDCHit_factory_Raw::GetConstant()! requested=%d , should be %ud", in_digihit->wire, WIRES_PER_PLANE);
      cerr << str << endl;
      throw JException(str);
   }
...
const double DFDCHit_factory_Raw::GetConstant(const fdc_digi_constants_t &the_table,
   ...
   if( (in_hit->gPlane <= 0) || (static_cast<unsigned int>(in_hit->gPlane) > FDC_NUM_PLANES)) {
      sprintf(str, "Bad gPlane # requested in DFDCHit_factory_Raw::GetConstant()! requested=%d , should be %ud", in_hit->gPlane, FDC_NUM_PLANES);
      cerr << str << endl;
      throw JException(str);
   }
   // strip and wire planes have different numbers of elements
   if( (in_hit->element <= 0) || (static_cast<unsigned int>(in_hit->element) > the_table[in_hit->gPlane].size())) {
      sprintf(str, "Bad element # requested in DFDCHit_factory_Raw::GetConstant()! requested=%d , should be %zu", in_hit->element, the_table[in_hit->gPlane].size());
      cerr << str << endl;
      throw JException(str);
   }
...
sprintf(str, "Bad strip # requested in DFDCHit_factory_Raw::GetConstant()! requested=%d , should be %ud", channel_info.fdc_cathodes.strip, STRIPS_PER_PLANE);
cerr << str << endl;
throw JException(str);
}
...
sprintf(str, "Bad strip # requested in DFDCHit_factory_Raw::GetConstant()! requested=%d , should be %ud", channel_info.fdc_wires.wire, WIRES_PER_PLANE);
cerr << str << endl;
throw JException(str);
}
...
sprintf(str, "Got bad detector type in DFDCHit_factory_Raw::GetConstant()! requested=%d", channel_info.module_type);
cerr << str << endl;
throw JException(str);
