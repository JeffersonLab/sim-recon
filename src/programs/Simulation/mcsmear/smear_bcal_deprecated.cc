...
static bool BCAL_INITIALIZED_DEPRECATED = false;
static double BCAL_mevPerPE=0.0;
static double BCAL_UNATTENUATE_TO_CENTER = 0.0;
static double BCAL_sigma_singlePE_sq = 0.0;
static double BCAL_sigma_ped_sq = 0.0;
...
static double BCAL_inner_thresh[DBCALGeometry::NBCALLAYSIN];
static double BCAL_outer_thresh[DBCALGeometry::NBCALLAYSOUT];

//number of SiPM's summed in one readout unit for each layer
static int Nsum_inner[DBCALGeometry::NBCALLAYSIN];
static int Nsum_outer[DBCALGeometry::NBCALLAYSOUT];


static map<int, DBCALReadoutChannel> bcal_fADCs; // key is DBCALGeometry::fADCId()
...
void SmearBCAL_DEPRECATED(s_HDDM_t *hddm_s)
{
	...
	if(!BCAL_INITIALIZED_DEPRECATED)bcalInit_DEPRECATED();
...
void bcalInit_DEPRECATED(void)
{
	...
	BCAL_INITIALIZED_DEPRECATED = true;
}
