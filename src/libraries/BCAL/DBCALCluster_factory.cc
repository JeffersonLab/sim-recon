	...
	while( seedThresh > minSeed ) {
		...
		for( vector< const DBCALPoint* >::iterator pt = points.begin();
			...
			for( vector< const DTrackWireBased* >::iterator trk = tracks.begin();
				...
				// deal with annoying 0/2pi BCAL behavior
...
DBCALCluster_factory::recycle_points( vector<const DBCALPoint*> usedPoints, vector<DBCALCluster*>& clusters) const{
	...
	//	cout << " used points energy = " << (*usedpt)->E() << endl;
	...
	//	cout << " overlap count = " << overlap_counter << endl;
...
//			cout << " best clust " << best_clust << " E = " << (*usedpt)->E() << endl;
			...
			if(best_clust==0){
				...
				for(unsigned int i = 0 ; i < associated_points.size(); i++){
					...
					if( point_match==0 && added_point==0 && deltaPhi == *min_phi){
						...
						added_point=1;
					//	cout << " point recycled E = " << (*usedpt)->E() << endl;
					}
...
DBCALCluster_factory::overlap( const DBCALCluster& highEClust,
	...
	double lowE_point_z = 0.;
	 ...
	 for(unsigned int i = 0 ; i < highE_points.size() ; i ++){
		...
		if( lowE_point_layer == highE_points[i]->layer() && fabs(lowE_point_z - highE_points[i]->z()) < 50. && ( lowE_global_sector+1 == highE_global_sector || lowE_global_sector-1 == highE_global_sector ) ) connected = 1; // clustesr that contain only a single point won't have any fit parameters and will make it hard for them to merge, this connected int will force a merge if a single point cluster is connected to a cluster without any points adjacent to it.
	}
...
//	return theta_match && phi_match && time_match;
	...
	else return ( ( delta_slope < 0.01 && delta_intercept < 2. && deltaPhi < 0.24) || connected == 1 ) ;
...
/*		summed_phi += p_phi;
		...
		summed_zphi += assoc_points[i]->z()*p_phi;
...
*/
	...
	//	cout << " slope = " << slope << " y int = " << y_intercept << " point z = " << summed_z << " tracked z = " << tracked_z << " point r = " << summed_r << " point phi = " << summed_phi*TMath::RadToDeg() << " track phi = " << tracked_phi*TMath::RadToDeg() << endl;
...
//	else fit_phi = slope*point->r() + y_intercept;
...
//	double inclusion_val2 = -0.0857*sep_term2 + 0.4;
