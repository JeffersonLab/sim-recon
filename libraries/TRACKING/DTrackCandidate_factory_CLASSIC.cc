// $Id: DTrackCandidate_factory_CLASSIC.cc 2673 2007-06-13 03:02:09Z davidl $
//
//    File: DTrackCandidate_factory_CLASSIC.cc
...
class TrkHitSort{
	public:
		bool operator()(Dmctrk_hit* const &hit1, Dmctrk_hit* const &hit2) const {
...
bool TrkHitSort_C(Dmctrk_hit* const &hit1, Dmctrk_hit* const &hit2) {
	return hit1->r > hit2->r;
}

bool TrkHitZSort_C(Dmctrk_hit* const &hit1, Dmctrk_hit* const &hit2) {
	return hit1->z < hit2->z;
}


//------------------
// DTrackCandidate_factory_CLASSIC
//------------------
DTrackCandidate_factory_CLASSIC::DTrackCandidate_factory_CLASSIC()
{
	// Set defaults
	MAX_SEED_DIST = 5.0;
	MAX_SEED_HITS = 10;
...
// init
//------------------
jerror_t DTrackCandidate_factory_CLASSIC::init(void)
{
	dgeom = NULL;
	bfield = NULL;
	...
	// DTrackCandidate_factory_CLASSIC object or not before creating them. If we
	// don't then the DTrackCandidate_factory_CLASSIC__THROWN class will recreate
	// them leading to warning messages from ROOT.
	if(typeid(this) == typeid(DTrackCandidate_factory_CLASSIC*)){
		char title[64];
...
// brun
//------------------
jerror_t DTrackCandidate_factory_CLASSIC::brun(JEventLoop *loop, int runnumber)
{
...
// fini
//------------------
jerror_t DTrackCandidate_factory_CLASSIC::fini(void)
{
...
// ClearEvent
//------------------
void DTrackCandidate_factory_CLASSIC::ClearEvent(void)
{
...
// GetTrkHits
//------------------
void DTrackCandidate_factory_CLASSIC::GetTrkHits(JEventLoop *loop)
{
	// Copy Hits into trkhits vector. The objects returned as
	// DTrackHit should really be Dmctrk_hit types. Dynamically
	// cast them back
	vector<const DTrackHit*> trackhits;
	loop->Get(trackhits, TRACKHIT_SOURCE.c_str());
	for(unsigned int i=0; i<trackhits.size(); i++){
		Dmctrk_hit *hit = (Dmctrk_hit*)dynamic_cast<const Dmctrk_hit*>(trackhits[i]);
		if(hit){
	...
	for(unsigned int i=0; i<trkhits.size(); i++){
		Dmctrk_hit *a = trkhits[i];
		
		// temporarily flag this hit to be ignored so FindClosestXY will work
		a->flags |= Dmctrk_hit::IGNORE;
		Dmctrk_hit *b = FindClosestXY(a);
		if(b){
			if(a->DistXY2(b) < XY_NOISE_CUT2)a->flags &= ~Dmctrk_hit::IGNORE;
		}
	}
}
...
// FindSeed
//------------------
int DTrackCandidate_factory_CLASSIC::FindSeed(void)
{
	...
	for(unsigned int i=0; i<trkhits_r_sorted.size(); i++){
		Dmctrk_hit *a = trkhits_r_sorted[i];
		if(!(a->flags & (Dmctrk_hit::USED | Dmctrk_hit::IGNORE))){
		
			// Clear IN_SEED bit flag all hits
			for(unsigned int j=0; j<trkhits.size(); j++){
				trkhits[j]->flags &= ~(Dmctrk_hit::IN_SEED);
			}
...
// TraceSeed
//------------------
int DTrackCandidate_factory_CLASSIC::TraceSeed(Dmctrk_hit *hit)
{
	...
	int N = 0;
	Dmctrk_hit *current_hit = hit;
	Dmctrk_hit *last_hit = NULL;
	do{
		N++;
		current_hit->flags |= Dmctrk_hit::IN_SEED;
		hits_in_seed.push_back(current_hit);
		if(hits_in_seed.size() >= MAX_SEED_HITS)break;
		Dmctrk_hit *a = FindClosestXY(current_hit);
		if(!a)break;
...
// FindClosestXY
//------------------
Dmctrk_hit* DTrackCandidate_factory_CLASSIC::FindClosestXY(Dmctrk_hit *hit)
{
	Dmctrk_hit *closest_hit = NULL;
	unsigned int mask = Dmctrk_hit::USED | Dmctrk_hit::IN_SEED | Dmctrk_hit::IGNORE;
	float d2_min = 1000.0;
	for(unsigned int i=0; i<trkhits.size(); i++){
		Dmctrk_hit *a = trkhits[i];
...
// FitSeed
//------------------
int DTrackCandidate_factory_CLASSIC::FitSeed(void)
{
	...
	for(unsigned int i=0; i<trkhits.size(); i++){
		Dmctrk_hit *a = trkhits[i];
		if(!(a->flags&Dmctrk_hit::IN_SEED))continue;
	...
	for(unsigned int i=0; i<trkhits.size(); i++){
		Dmctrk_hit *a = trkhits[i];
		a->flags &= ~(Dmctrk_hit::ON_CIRCLE | Dmctrk_hit::ON_LINE | Dmctrk_hit::ON_TRACK);
		if(a->flags&Dmctrk_hit::USED)continue;
		...
		if(fabs(r0-r) <= MAX_CIRCLE_DIST){
			a->flags |= Dmctrk_hit::ON_CIRCLE;
			hits_on_circle.push_back(a);
			if(a->flags & Dmctrk_hit::IN_SEED)N_in_seed_and_on_circle++;
		}
	}
...
// FindLineHits
//------------------
int DTrackCandidate_factory_CLASSIC::FindLineHits(void)
{
...
// FindPhiZAngle
//------------------
int DTrackCandidate_factory_CLASSIC::FindPhiZAngle(void)
{
...
// Fill_phi_circle
//------------------
void DTrackCandidate_factory_CLASSIC::Fill_phi_circle(vector<Dmctrk_hit*> hits, float X, float Y)
{
	/// Fill in the phi_circle attribute for all Dmctrk_hits in "hits" by
...
// FindZvertex
//------------------
int DTrackCandidate_factory_CLASSIC::FindZvertex(void)
{
...
// FitTrack
//------------------
int DTrackCandidate_factory_CLASSIC::FitTrack(void)
{
...
// MarkTrackHits
//------------------
int DTrackCandidate_factory_CLASSIC::MarkTrackHits(DTrackCandidate *trackcandidate, DQuickFit *fit)
{
...
my_phizangle = phizangle;
	...
	for(unsigned int i=0; i<trkhits.size(); i++){
		...
		a->flags |= Dmctrk_hit::ON_TRACK | Dmctrk_hit::USED;
		hits_on_track.push_back(a);
		if(a->flags & Dmctrk_hit::IN_SEED)N_in_seed_and_on_track++;
...
// toString
//------------------
const string DTrackCandidate_factory_CLASSIC::toString(void)
{
...
// DumpHits
//------------------
void DTrackCandidate_factory_CLASSIC::DumpHits(int current_seed_number, string stage)
{
	// Count statistics for flags
	int Nhits = 0;
	int in_seed=0, used=0, on_circle=0, ignore=0 ,on_line=0, on_track=0;
	for(unsigned int i=0; i<trkhits.size(); i++){
		const Dmctrk_hit *a = trkhits[i];
		Nhits++;
		if(a->flags&Dmctrk_hit::IN_SEED)in_seed++;
		if(a->flags&Dmctrk_hit::USED)used++;
		if(a->flags&Dmctrk_hit::ON_CIRCLE)on_circle++;
		if(a->flags&Dmctrk_hit::IGNORE)ignore++;
		if(a->flags&Dmctrk_hit::ON_LINE)on_line++;
		if(a->flags&Dmctrk_hit::ON_TRACK)on_track++;
	}
	...
	cout<<"hits:"<<endl;
	for(unsigned int i=0; i<trkhits.size(); i++){
		Dmctrk_hit *t = trkhits[i];
		
		cout<<i<<" flags:"<<t->flags<<" = ";
		if(t->flags == 0x0)cout<<" NONE";
		if(t->flags & Dmctrk_hit::USED)cout<<" USED";
		if(t->flags & Dmctrk_hit::IN_SEED)cout<<" IN_SEED";
		if(t->flags & Dmctrk_hit::ON_CIRCLE)cout<<" ON_CIRCLE";
		if(t->flags & Dmctrk_hit::ON_LINE)cout<<" ON_LINE";
		if(t->flags & Dmctrk_hit::ON_TRACK)cout<<" ON_TRACK";
		if(t->flags & Dmctrk_hit::IGNORE)cout<<" IGNORE";
		cout<<endl;
