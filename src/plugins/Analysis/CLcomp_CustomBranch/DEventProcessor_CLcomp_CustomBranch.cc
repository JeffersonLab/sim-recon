...
extern "C"
{
	void InitPlugin(JApplication *locApplication)
	{
		...
		locApplication->AddProcessor(new DEventProcessor_CLcomp_CustomBranch()); //register this plugin
		locApplication->AddFactoryGenerator(new DFactoryGenerator_CLcomp_CustomBranch()); //register the factory generator
	}
} // "C"
...
// init
//------------------
jerror_t DEventProcessor_CLcomp_CustomBranch::init(void)
{
	// This is called once at program startup.

	/*
	//OPTIONAL: Create an EventStore skim.  
	string locSkimFileName = "p2k.idxa";
...
// brun
//------------------
jerror_t DEventProcessor_CLcomp_CustomBranch::brun(jana::JEventLoop* locEventLoop, int32_t locRunNumber)
{
...
// evnt
//------------------
jerror_t DEventProcessor_CLcomp_CustomBranch::evnt(jana::JEventLoop* locEventLoop, uint64_t locEventNumber)
{
	...
	*/
	...
	for(size_t loc_i = 0; loc_i < locParticleCombos.size(); ++loc_i)
	{
		...
		if(locParticleCombo->Get_Reaction()->Get_ReactionName() != "p2k")
	...
	*/
	...
	for(size_t loc_i = 0; loc_i < locAnalysisResultsVector.size(); ++loc_i)
	{
		...
		if(locAnalysisResults->Get_Reaction()->Get_ReactionName() != "p2k")
	...
	*/
	...
	for(size_t loc_i = 0; loc_i < locAnalysisResultsVector.size(); ++loc_i)
	{
		...
		if(!locPassedParticleCombos.empty())
			locEventWriterREST->Write_RESTEvent(locEventLoop, "p2k"); //string is part of output file name
	}
	*/
	...
	for(size_t loc_i = 0; loc_i < locAnalysisResultsVector.size(); ++loc_i)
	{
		...
		japp->WriteLock("p2k.idxa"); //Lock is unique to this output file
		{
			dEventStoreSkimStream << locRunNumber << " " << locEventNumber << " " << locUniqueID << endl;
		}
		japp->Unlock("p2k.idxa");
	}
	*/
...
int DEventProcessor_CLcomp_CustomBranch::Get_FileNumber(JEventLoop* locEventLoop) const
{
...
// erun
//------------------
jerror_t DEventProcessor_CLcomp_CustomBranch::erun(void)
{
...
// fini
//------------------
jerror_t DEventProcessor_CLcomp_CustomBranch::fini(void)
{
