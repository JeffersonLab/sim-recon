...
jerror_t JEventProcessor_CDC_expert::init(void) {
  ...
  const Int_t IMAX = 400000;  //max for raw integral
  ...
  */
  ...
  const Int_t TMAX = 1500;    //max for t histograms, ns
...
jerror_t JEventProcessor_CDC_expert::brun(JEventLoop *eventLoop, int runnumber) {
...
jerror_t JEventProcessor_CDC_expert::evnt(JEventLoop *eventLoop, int eventnumber) {
  ...
  for(uint32_t i=0; i<digihits.size(); i++) {
    ...
    if (windat) {
      ...
      if (windat->samples.size()>=NPEDSAMPLES) {
        ...
        if (winped > 0) {
          ...
          if (pi->rocid == 25) cdc_windata_ped_roc25->Fill(100*pi->slot + pi->channel,winped);
          if (pi->rocid == 26) cdc_windata_ped_roc26->Fill(100*pi->slot + pi->channel,winped);
          if (pi->rocid == 27) cdc_windata_ped_roc27->Fill(100*pi->slot + pi->channel,winped);
          if (pi->rocid == 28) cdc_windata_ped_roc28->Fill(100*pi->slot + pi->channel,winped);
    ...
    if((digihit->pulse_integral>0)||(digihit->pulse_time>0)) {
      ...
      if (a>p) cdc_raw_amp_ring[ring]->Fill(straw,a-p);
