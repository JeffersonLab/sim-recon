void GetData(TString fname,double *y,double *dy,double &min,double &max) {
    ...
    for (int i=0;i<N;i++) {
        char sep;int n; double t,dt,sigma;
        fin >> n >> sep >> n >> sep >> n >> sep >> t >> sep >> dt >> sep >> sigma;
        y[i] = t; dy[i] = dt;
        if (t>max) max = t;
        if (t<min) min = t;
...
double func(double *x,double *par) {
    //return par[0]+par[1]*log(x[0])+par[2]*log(x[0])*log(x[0]);
...
void WriteTimewalkFitResults(ofstream &fout,ofstream &fout_ccdb,int counter,double *y,double *dy,double min,double max) {
    ...
    gr.Draw("AP");
    if (f.GetNDF() == 0 || f.GetChisquare()/f.GetNDF() > 150.0) {
