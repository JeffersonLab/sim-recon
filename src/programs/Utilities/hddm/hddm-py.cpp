...
int main(int argC, char* argV[])
{
   ...
   builder.pyFile <<
   "/*\n"
   " * pyhddm_" << classPrefix << ".hpp - DO NOT EDIT THIS FILE\n"
   " *\n"
   ...
   "      return NULL;\n"
   "   }\n"
   "   HDDM_ElementList<HDDM_Element> *oldlist = list_obj->list;\n"
   "   HDDM_ElementList<HDDM_Element> *newlist;\n"
   "   newlist = new HDDM_ElementList<HDDM_Element>(oldlist->add(count, start));\n"
   "   list_obj->list = newlist;\n"
   "   delete oldlist;\n"
   "   Py_INCREF(self);\n"
   "   return self;\n"
   "}\n"
   ...
   "{\n"
   "   PyObject *start = Py_BuildValue(\"k\", self->streampos->block_start);\n"
   "   Py_INCREF(start);\n"
   "   return start;\n"
   "}\n"
   "\n"
   ...
   "{\n"
   "   PyObject *offset = Py_BuildValue(\"I\", self->streampos->block_offset);\n"
   "   Py_INCREF(offset);\n"
   "   return offset;\n"
   "}\n"
   "\n"
   ...
   "{\n"
   "   PyObject *status = Py_BuildValue(\"I\", self->streampos->block_status);\n"
   "   Py_INCREF(status);\n"
   "   return status;\n"
   "}\n"
   "\n"
   ...
   "{\n"
   "   PyObject *flags = Py_BuildValue(\"i\", self->ostr->getCompression());\n"
   "   Py_INCREF(flags);\n"
   "   return flags;\n"
   "}\n"
   "\n"
   ...
   "{\n"
   "   PyObject *flags = Py_BuildValue(\"i\", self->istr->getCompression());\n"
   "   Py_INCREF(flags);\n"
   "   return flags;\n"
   "}\n"
   "\n"
   ...
   "{\n"
   "   PyObject *flags = Py_BuildValue(\"i\", self->istr->getIntegrityChecks());\n"
   "   Py_INCREF(flags);\n"
   "   return flags;\n"
   "}\n"
   "\n"
   ...
   "{\n"
   "   if (value->ob_type != &_streamposition_type)\n"
   "   {\n"
...
void CodeBuilder::writeClassdef(DOMElement* el)
{
   ...
   for (citer = children[tagS].begin(); citer != children[tagS].end(); ++citer)
   {
      ...
      pyFile << "static PyObject*\n" << getS <<
                ...
                "   ((_" << cnameS.simpleType() << 
                "*)obj)->elem = new " << cnameS.simpleType() << "("
                "me->elem->get" << cnameS.simpleType() << 
                ((rep > 1)? "(index)" : "()") << ");\n"
                "   Py_INCREF(obj);\n"
                "   return obj;\n"
                "}\n\n"
