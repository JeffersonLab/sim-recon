...
class Df250PulseTime:public jana::JObject{
	...
	public:
		...
		Df250PulseTime(uint32_t rocid=0, uint32_t slot=0, uint32_t channel=0, uint32_t itrigger=0, uint32_t pulse_number=0, uint32_t quality_factor=0, uint32_t time=0):rocid(rocid),slot(slot),channel(channel),itrigger(itrigger),pulse_number(pulse_number),quality_factor(quality_factor),time(time){}
