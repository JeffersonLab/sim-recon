...
int PhotonBeamType=0;   // 0=monoenergetic, 1=realistic beam profile
char PhotonBeamFile[2048] = "cobrems.root";
TH1 *PhotonHist = NULL;
double EBeamMin = 7.;
double EBeamMax = 11.8;
/***********************/
/* Declarations         */
/***********************/
/*
...
void PrintUsage(char *processName)
{
  ...
  fprintf(stderr,"\t-b<type> Photon beam type (0=monoenergetic, 1=read from file). \n");
  fprintf(stderr,"\t-B<filename> ROOT file name which stores the beam shape. \n");
  fprintf(stderr,"\t-E<maxE> Maximum beam energy. \n");
  fprintf(stderr,"\t-e<minE> Minimum beam energy. \n");
...
int main(int argc,char **argv)
{
  ...
  if (argc == 1){
	...
	case 'r':
	  ...
	  break;
	case 'E':
	  EBeamMax = atoi(++argptr);
	  fprintf(stderr,"Using EBeamMax: %d\n",EBeamMax);
	  break;
	case 'e':
	  EBeamMin = atoi(++argptr);
	  fprintf(stderr,"Using EBeamMin: %d\n",EBeamMin);
	  break;
	case 'b':
	  PhotonBeamType = atoi(++argptr);
	  fprintf(stderr,"Using PhotonBeamType: %d\n",PhotonBeamType);
	  break;
	case 'B':
	  strcpy(PhotonBeamFile,++argptr);
	  fprintf(stderr,"Using PhotonBeamFile: %s\n",PhotonBeamFile);
	  break;
	case 's':
  ...
  TRandom3 rand;
  rand.SetSeed(RandomSeed);    // need to set this
...
/* Fill particle information */
  ...
  // load the photon beam info
  //int eBinLow = hGvsE->GetXaxis()->FindBin(eLower);
  //int eBinHigh = hGvsE->GetXaxis()->FindBin(eUpper);
  double gMax = 0;
  if(PhotonBeamType) {
  	TFile f(PhotonBeamFile);
  	PhotonHist = (TH1*)f.Get("cobrem_vs_E")->Clone();
	//PhotonHist->Print("base");
	PhotonHist->SetDirectory(0);
  	//f.Close();

  	int eBinLow = PhotonHist->GetXaxis()->FindBin(EBeamMin);
  	int eBinHigh = PhotonHist->GetXaxis()->FindBin(EBeamMax);
  	PhotonHist->GetXaxis()->SetRange(eBinLow,eBinHigh);
  	gMax = PhotonHist->GetMaximum();
  }
  ...
  for(i=0;i<npart;i++){ /* read particle information*/
	...
	particle[part].particleID = static_cast<Particle_t>(atoi(token));/* see particleType.h */
      token=strtok(NULL," ");
      if(!(*token == '*')){
    ...
    if (Debug) fprintf(stderr,"In main do loop ... %d \n",naccepted);
  	  ...
  	  if(PhotonBeamType) {
      	...
      	while(testValY>yVal){//Monte Carlo the event to get brem spectrum
          	eGamma = rand.Uniform(EBeamMin,EBeamMax); //Grab a photon energy
          	testValY = rand.Uniform(0.0,yMax); //Grab a test value                  
          	eBin = PhotonHist->GetXaxis()->FindBin(eGamma);
          	yVal = PhotonHist->GetBinContent(eBin);
      	}
      	//hGvsEout->Fill(eGamma);
      	
      	// recalculate CM
      	beam.p.space.z = eGamma;
      	beam.p.t = energy(beam.mass,&(beam.p.space));
    	sqrt_s = sqrt( SQ(beam.mass) +SQ(target.mass) + 2.0*beam.p.t * target.p.t);
