#ifndef _DBCALTruthHit_factory_
#define _DBCALTruthHit_factory_
...
class DBCALTruthCell;

class DBCALTruthHit_factory : public JFactory<DBCALTruthHit> {

 public:
  DBCALTruthHit_factory() {
    PRINTCALIBRATION = false;
    if(gPARMS){
      gPARMS->SetDefaultParameter("BCALTruthHit:PRINTCALIBRATION", PRINTCALIBRATION, "Print the calibration parameters.");
    }
  }
  ~DBCALTruthHit_factory() {}
