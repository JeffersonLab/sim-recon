...
jerror_t MyProcessor::evnt(JEventLoop *eventLoop, int eventnumber)
{
  ...
  { 
    ...
    pim[1] = kd_finalState[3].lorentzMomentum();
    p = kd_finalState[4].lorentzMomentum();

    mm2 = (beam + targ - p - pip[0] - pim[0] - pip[1] - pim[1]).M2();
    ks[0] = (pip[0] + pim[0]).M();
    ks[1] = (pip[1] + pim[1]).M();
    ...
    constraintParticles.push_back(2); // pi+ 0
    constraintParticles.push_back(3); // pi- 0
    kfit->SetMassConstraint(0.494,constraintParticles);
    kfit->Fit();
    ...
    pimfit[1] = kd_finalState_post[3].lorentzMomentum();
    pfit = kd_finalState_post[4].lorentzMomentum();

    mm2fit = (beamfit + targfit - pfit - pipfit[0] - pimfit[0] - pipfit[1] - pimfit[1]).M2();
    ...
    for(int j=0;j<5;j++)
    {
      if(cl>clCut[j]) 
      {
        ...
        for(int k=0;k<15;k++) hpulls[1][0][j][k]->Fill(pulls[k]);
      }
    }
