...
namespace xstream
{
	namespace base64
	{
		...
		ostreambuf::ostreambuf(std::streambuf* sb, const unsigned int w, const char c):_sb(sb),delim(c),delim_w(w),col(0) {
		...
		int ostreambuf::write(const char* buf, const size_t len){
		...
		static inline char index(const char c) {
		...
		istreambuf::istreambuf(std::streambuf* sb, const unsigned int d_w, const char d):_sb(sb),end(false),delim(d),delim_w(d_w),col(0) {
