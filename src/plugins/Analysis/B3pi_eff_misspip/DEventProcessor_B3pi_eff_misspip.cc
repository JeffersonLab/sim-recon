...
extern "C"
{
	void InitPlugin(JApplication *locApplication)
	{
		...
		locApplication->AddProcessor(new DEventProcessor_B3pi_eff_misspip()); //register this plugin
		locApplication->AddFactoryGenerator(new DFactoryGenerator_B3pi_eff_misspip()); //register the factory generator
	}
} // "C"
...
// init
//------------------
jerror_t DEventProcessor_B3pi_eff_misspip::init(void)
{
	// This is called once at program startup.

	/*
	//OPTIONAL: Create an EventStore skim.  
	string locSkimFileName = "B3pi_eff_misspip.idxa";
...
// brun
//------------------
jerror_t DEventProcessor_B3pi_eff_misspip::brun(jana::JEventLoop* locEventLoop, int32_t locRunNumber)
{
...
// evnt
//------------------
jerror_t DEventProcessor_B3pi_eff_misspip::evnt(jana::JEventLoop* locEventLoop, uint64_t locEventNumber)
{
	...
	locEventWriterROOT->Fill_DataTrees(locEventLoop, "B3pi_eff_misspip");
	...
	*/
	...
	for(size_t loc_i = 0; loc_i < locParticleCombos.size(); ++loc_i)
	{
		...
		if(locParticleCombo->Get_Reaction()->Get_ReactionName() != "B3pi_eff_misspip")
	...
	*/
	...
	for(size_t loc_i = 0; loc_i < locAnalysisResultsVector.size(); ++loc_i)
	{
		...
		if(locAnalysisResults->Get_Reaction()->Get_ReactionName() != "B3pi_eff_misspip")
	...
	*/
	...
	for(size_t loc_i = 0; loc_i < locAnalysisResultsVector.size(); ++loc_i)
	{
		...
		if(!locPassedParticleCombos.empty())
			locEventWriterREST->Write_RESTEvent(locEventLoop, "B3pi_eff_misspip"); //string is part of output file name
	}
	*/
	...
	for(size_t loc_i = 0; loc_i < locAnalysisResultsVector.size(); ++loc_i)
	{
		...
		japp->WriteLock("B3pi_eff_misspip.idxa"); //Lock is unique to this output file
		{
			dEventStoreSkimStream << locRunNumber << " " << locEventNumber << " " << locUniqueID << endl;
		}
		japp->Unlock("B3pi_eff_misspip.idxa");
	}
	*/
...
int DEventProcessor_B3pi_eff_misspip::Get_FileNumber(JEventLoop* locEventLoop) const
{
...
// erun
//------------------
jerror_t DEventProcessor_B3pi_eff_misspip::erun(void)
{
...
// fini
//------------------
jerror_t DEventProcessor_B3pi_eff_misspip::fini(void)
{
