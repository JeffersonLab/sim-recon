...
jerror_t DTOFGeometry_factory::brun(jana::JEventLoop *loop, int runnumber)
{
  ...
  bool got_sc=locGeometry->Get("//section/composition/posXYZ[@volume='ForwardTOF']/@X_Y_Z",tof_face);
  vector<double>tof_plane0;
  got_sc=locGeometry->Get("//composition[@name='ForwardTOF']/posXYZ[@volume='forwardTOF']/@X_Y_Z/plane[@value='0']", tof_plane0);
  vector<double>tof_plane1;
  got_sc=locGeometry->Get("//composition[@name='ForwardTOF']/posXYZ[@volume='forwardTOF']/@X_Y_Z/plane[@value='1']", tof_plane1);
  ...
  // First 19 long bars
  got_sc=locGeometry->Get("//composition[@name='forwardTOF_bottom1']/mposY/@Y0",y0);
  got_sc=locGeometry->Get("//composition[@name='forwardTOF_bottom1']/mposY/@dY",dy);
  vector<double>tof_bottom1;
  got_sc=locGeometry->Get("//composition[@name='forwardTOF']/posXYZ[@volume='forwardTOF_bottom1']/@X_Y_Z",tof_bottom1);  
  for (unsigned int k=1;k<20;k++){
    myDTOFGeometry->YPOS[k]=y0+tof_bottom1[1]+dy*double(k-1);
  }
  
  // two narrow long bars
  got_sc=locGeometry->Get("//composition[@name='forwardTOF_bottom2']/mposY/@Y0",y0);
  got_sc=locGeometry->Get("//composition[@name='forwardTOF_bottom2']/mposY/@dY",dy);
  vector<double>tof_bottom2;
  got_sc=locGeometry->Get("//composition[@name='forwardTOF']/posXYZ[@volume='forwardTOF_bottom2']/@X_Y_Z",tof_bottom2);  
  for (unsigned int k=20;k<22;k++){
    myDTOFGeometry->YPOS[k]=y0+tof_bottom2[1]+dy*double(k-20);
  }

  // two short wide bars
  got_sc=locGeometry->Get("//composition[@name='forwardTOF_north']/mposY/@Y0",y0);
  got_sc=locGeometry->Get("//composition[@name='forwardTOF_north']/mposY/@dY",dy);
  vector<double>tof_north;
  got_sc=locGeometry->Get("//composition[@name='forwardTOF']/posXYZ[@volume='forwardTOF_north']/@X_Y_Z",tof_north);  
  for (unsigned int k=22;k<24;k++){
    myDTOFGeometry->YPOS[k]=y0+tof_north[1]+dy*double(k-22);
  }

  // two narrow long bars
  got_sc=locGeometry->Get("//composition[@name='forwardTOF_top2']/mposY/@Y0",y0);
  got_sc=locGeometry->Get("//composition[@name='forwardTOF_top2']/mposY/@dY",dy);
  vector<double>tof_top2;
  got_sc=locGeometry->Get("//composition[@name='forwardTOF']/posXYZ[@volume='forwardTOF_top2']/@X_Y_Z",tof_top2);  
  for (unsigned int k=24;k<26;k++){
    myDTOFGeometry->YPOS[k]=y0+tof_top2[1]+dy*double(k-24);
  }

  // Last 19 long bars
  got_sc=locGeometry->Get("//composition[@name='forwardTOF_top1']/mposY/@Y0",y0);
  got_sc=locGeometry->Get("//composition[@name='forwardTOF_top1']/mposY/@dY",dy);
  vector<double>tof_top1;
  got_sc=locGeometry->Get("//composition[@name='forwardTOF']/posXYZ[@volume='forwardTOF_top1']/@X_Y_Z",tof_top1);  
  for (unsigned int k=26;k<45;k++){
    myDTOFGeometry->YPOS[k]=y0+tof_top1[1]+dy*double(k-26);
  }

  // two more short wide bars
  got_sc=locGeometry->Get("//composition[@name='forwardTOF_south']/mposY/@Y0",y0);
  got_sc=locGeometry->Get("//composition[@name='forwardTOF_south']/mposY/@dY",dy);
  vector<double>tof_south;
  got_sc=locGeometry->Get("//composition[@name='forwardTOF']/posXYZ[@volume='forwardTOF_south']/@X_Y_Z",tof_south);  
