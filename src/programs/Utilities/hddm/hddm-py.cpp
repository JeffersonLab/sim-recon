...
int main(int argC, char* argV[])
{
   ...
   "{\n"
   "   if (self->elem != 0) {\n"
   "      LOG_DEALLOC(self->ob_type, 0, self->host == (PyObject*)self);\n"
   ...
   "{\n"
   "   if (self->list != 0) {\n"
   "      LOG_DEALLOC(self->ob_type, self->subtype, self->borrowed == 0);\n"
   "      if (self->borrowed == 0)\n"
   "         delete self->list;\n"
   "      My_DECREF(self->host);\n"
   "   }\n"
   ...
   "   My_INCREF(me->host);\n"
   "   LOG_NEW(self->ob_type, me->subtype, 1);\n"
   "   return list;\n"
   "}\n"
   "\n"
   "static PyObject *\n"
   ...
   "      return NULL;\n"
   "   }\n"
   "   LOG_NEW(record_obj->ob_type, 0, 1);\n"
   ...
   for (unsigned int n = 0; n < myAttr->getLength(); n++)
   {
      ...
      else if (typeS == "string")
      {
         pyFile << "static int\n" <<
                   ...
                   " *self, PyObject *value, void *closure)\n"
                   "{\n"
                   "   char *var = PyString_AsString(value);\n"
                   "   if (var == 0) {\n"
                   "      return -1;\n"
                   "   }\n"
