...
jerror_t DTrackFitterKalman::PropagateForwardCDC(int length,int &index,double z,
  ...
  if (my_fdchits.size()==0 && temp.pos.z()>endplate_z){
  ...
  if (my_fdchits.size()==0 && temp.pos.z()>endplate_z) dEdx=0.;
  else
...
jerror_t DTrackFitterKalman::SetReferenceTrajectory(DMatrix &S){   
  ...
  if (DEBUG_LEVEL==2){
    printf("--------------- %d %d\n",forward_traj_length, (int)forward_traj.size());
...
jerror_t DTrackFitterKalman::GetProcessNoiseCentral(double mass_hyp,double ds,
...
jerror_t DTrackFitterKalman::GetProcessNoise(double mass_hyp,double ds,double z,
...
double DTrackFitterKalman::GetdEdx(double M,double q_over_p,double Z,
			      double A, double rho){
  if (rho<=0.) return 0.;
  //return 0;
...
jerror_t DTrackFitterKalman::KalmanCentral(double mass_hyp,double anneal_factor,
				      DMatrix &Sc,DMatrix &Cc,
...
jerror_t DTrackFitterKalman::KalmanForwardCDC(double mass_hyp,double anneal,
					 DMatrix &S, 
  ...
  */
  ...
  for (unsigned int k=1;k<forward_traj_cdc.size()-1;k++){
    ...
    */
    ...
    // Check if the doca is no longer decreasing
    if (DEBUG_LEVEL==2){
      printf("Ring %d straw %d\n",my_cdchits[cdc_index]->ring,
	     my_cdchits[cdc_index]->straw);
	...
	//s=forward_traj_cdc[k].s;
	dm=CDC_DRIFT_SPEED*(my_cdchits[cdc_index]->t-s*one_over_beta
				   /SPEED_OF_LIGHT);
	// variance
	V=0.000225*anneal;
      }
...
// through the field
jerror_t DTrackFitterKalman::SwimToPlane(double z_start,double z_end, DMatrix &S,
				    DMatrix &C){
...
jerror_t DTrackFitterKalman::SwimToRadius(DVector3 &pos,double Rf,DMatrix &Sc){
  ...
  double ds=0.25;
  double dedx=0.;
  DVector3 origin=my_cdchits[0]->origin;
  DVector3 dir=my_cdchits[0]->dir;
  while (R<Rf){ 
    // Get dEdx for this step
    //double tanl=Sc(state_tanl,0);
    //double cosl=cos(atan(tanl));
