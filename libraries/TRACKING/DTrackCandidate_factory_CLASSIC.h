// $Id: DTrackCandidate_factory_CLASSIC.h 2673 2007-06-13 03:02:09Z davidl $
//
//    File: DTrackCandidate_factory_CLASSIC.h
// Created: Mon Jul 18 15:23:04 EDT 2005
// Creator: davidl (on Darwin wire129.jlab.org 7.8.0 powerpc)
//

#ifndef _DTrackCandidate_factory_CLASSIC_
#define _DTrackCandidate_factory_CLASSIC_
...
class DTrackCandidate_factory_CLASSIC:public JFactory<DTrackCandidate>{

	/// This class is a straight copy of the original DTrackCandidate factory
	/// which uses "truth tags". It is here to preserve the code while the
	/// new finder is developed using only hit information.

	public:
		DTrackCandidate_factory_CLASSIC();
		~DTrackCandidate_factory_CLASSIC(){};
		const string toString(void);
		virtual const char* Tag(void){return "CLASSIC";}
		static void Fill_phi_circle(vector<Dmctrk_hit*> hits, float x0, float y0);
		
		vector<Dmctrk_hit*>& Get_trkhits(void){return trkhits;}
		vector<vector<Dmctrk_hit*> >& Get_dbg_in_seed(void){return dbg_in_seed;}
		vector<vector<Dmctrk_hit*> >& Get_dbg_hoc(void){return dbg_hoc;}
		vector<vector<Dmctrk_hit*> >& Get_dbg_hol(void){return dbg_hol;}
		vector<vector<Dmctrk_hit*> >& Get_dbg_hot(void){return dbg_hot;}
	...
	protected:
		...
		int FindSeed(void);
		int TraceSeed(Dmctrk_hit *hit);
		Dmctrk_hit* FindClosestXY(Dmctrk_hit *hit);
		int FitSeed(void);
		...
		inline void ChopSeed(void){if(hits_in_seed.size()>0)hits_in_seed[0]->flags |= Dmctrk_hit::IGNORE;}
		...
		vector<Dmctrk_hit*> trkhits; // sorted by z
		vector<Dmctrk_hit*> trkhits_r_sorted; // sorted by dist. from beam line
		vector<Dmctrk_hit*> hits_in_seed;
		vector<Dmctrk_hit*> hits_on_circle;
		vector<Dmctrk_hit*> hits_on_line;
		vector<Dmctrk_hit*> hits_on_track;
		vector<vector<Dmctrk_hit*> > dbg_in_seed;
		vector<vector<Dmctrk_hit*> > dbg_hoc;
		vector<vector<Dmctrk_hit*> > dbg_hol;
		vector<vector<Dmctrk_hit*> > dbg_hot;
...
#endif // _DTrackCandidate_factory_CLASSIC_
