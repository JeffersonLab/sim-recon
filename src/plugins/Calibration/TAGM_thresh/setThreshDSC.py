...
infile = open('roctagm2_dsc_spring_2017_v3.cnf','r')
threshfile = open('tagm-thresh.txt','r')
outfile = open('threshDSC.cnf','w')
thresh = [] #thresholds based on pulse integral

for line in threshfile:
	col = int(line.split()[1])
	# 1000 mV range in fADC has 4096 adc
	thr = int(float(line.split()[2])/4.3*1000/4096)
...
for line in infile:
	if (line.startswith('DSC2_ALLCH_THR')):
		counter += 1
		print counter
		outfile.write('DSC2_ALLCH_THR   ')
		for i in range(16):
			if (counter == 7 and i > 9 and i < 15):
				# Set unused channels to 200 mV
				outfile.write(str(200) + '   ')
			elif (counter == 7 and i == 15):
				outfile.write(str(200) + '\n')
			elif (i == 15):
				outfile.write(str(threshTT[counter*16+i]) + '\n')
			else:
				outfile.write(str(threshTT[counter*16+i]) + '   ')
	else:
		outfile.write(line)
