// $Id$
//
//    File: DEventProcessor_B3pi_eff_missgamma.cc
...
jerror_t DEventProcessor_B3pi_eff_missgamma::init(void)
{
	...
	string locSkimFileName = "B3pi_eff_missgamma.idxa";
...
jerror_t DEventProcessor_B3pi_eff_missgamma::evnt(jana::JEventLoop* locEventLoop, uint64_t locEventNumber)
{
	...
	locEventLoop->GetSingle(locEventWriterROOT);
        ...
        {
                has_omega = has_omega || (locMCThrowns[i]->PID() == omega);
        }
	...
	*/
	...
	for(size_t loc_i = 0; loc_i < locParticleCombos.size(); ++loc_i)
	{
		...
		if(locParticleCombo->Get_Reaction()->Get_ReactionName() != "B3pi_eff_missgamma")
	...
	*/
	...
	for(size_t loc_i = 0; loc_i < locAnalysisResultsVector.size(); ++loc_i)
	{
		...
		if(locAnalysisResults->Get_Reaction()->Get_ReactionName() != "B3pi_eff_missgamma")
...
/*
	...
	for(size_t loc_i = 0; loc_i < locAnalysisResultsVector.size(); ++loc_i)
	{
		...
		japp->WriteLock("B3pi_eff_missgamma.idxa"); //Lock is unique to this output file
		{
