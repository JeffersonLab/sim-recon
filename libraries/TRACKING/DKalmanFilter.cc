...
jerror_t DKalmanFilter::SetCDCForwardReferenceTrajectory(DMatrix &S,DMatrix &C){
  ...
  double my_endz=Z_MAX-STEP_SIZE;
  double beta2=1.;
  ...
  for (unsigned int m=0;m<cdchits.size();m++){
	...
	break;
      ...
      beta2=1./(1.+MASS*MASS*S(state_q_over_p,0)*S(state_q_over_p,0));
      if (temp.density>0.)
...
jerror_t DKalmanFilter::SetCDCReferenceTrajectory(DMatrix &Sc,DMatrix &Cc){
  ...
  while(r<R_MAX && pos.z()<Z_MAX && pos.z()>Z_MIN){
    ...
    // get material properties from the Root Geometry
    if (RootGeom->FindMat(pos,temp.density,temp.A,temp.Z,temp.X0)!=NOERROR) break;
...
jerror_t DKalmanFilter::KalmanLoop(double mass_hyp,int pass){
  ...
  if (fdchits.size()>0){   
    ...
    for (int iter2=0;iter2<3;iter2++){    
      ...
      // first
      if (cdchits.size()>0){
	SetCDCForwardReferenceTrajectory(S,C0);
      }
      
      // Swim once through the field out to the most upstream FDC hit
      SetReferenceTrajectory(S,C0);
      C0=C;
...
jerror_t DKalmanFilter::KalmanCentral(double mass_hyp,double anneal_factor,
  ...
  for (unsigned int k=1;k<central_traj.size();k++){
	...
	if (myds>0){
	  // Multiple scattering    
	  GetProcessNoiseCentral(MASS,myds,pos0,central_traj[k].X0,S0,Q);
	  Cc=Cc+Q;
	}
	...
	Sc=S0+J*(Sc-Stemp); 
      ...
      if (ds3>0){
	// Multiple scattering    
	GetProcessNoiseCentral(MASS,ds3,pos0,central_traj[k].X0,S0,Q);
	Cc=Cc+Q;
      }
      for (unsigned int i=0;i<5;i++){
	for (unsigned int j=0;j<5;j++){
